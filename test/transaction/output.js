...
const {Script, Transaction, errors} = pqccore
const {BN} = pqccore.crypto
const {BufferWriter, BufferReader} = pqccore.encoding
...
describe('Output', () => {
  ...
  it('can be assigned a satoshi amount in big number', () => {
    ...
    newOutput.glv.should.equal(100);
  });
  ...
  describe('will error if output is not a positive integer', () => {
    it('-100', () => {
      ...
      }).should.throw('Output glv is not a natural number');
    });
  ...
  it('#toObject roundtrip will handle an invalid (null) script', () => {
    ...
    const br = new pqccore.encoding.BufferReader(invalidOutputScript);
