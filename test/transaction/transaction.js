...
const unsupportedTxObj = '{"version":1,"inputs":[{"prevTxId":"a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458","outputIndex":0,"sequenceNumber":4294967295,"script":"OP_1","output":{"glv":1020000,"script":"OP_1 OP_ADD OP_2 OP_EQUAL"}}],"outputs":[{"glv":1010000,"script":"OP_DUP OP_HASH160 20 0x7821c0a3768aa9d1a37e16cf76002aef5373f1a8 OP_EQUALVERIFY OP_CHECKSIG"}],"nLockTime":0}';
...
describe('Transaction', () => {
  ...
  const testTransaction = new Transaction()
    ...
    .to('Lfci7ooSc31oNijNG9zDeHBBHJddxrPEKX', testAmount - 10000);

  it('can serialize to a plain javascript object', () => {
    const object = testTransaction.toObject();
    object.inputs[0].output.glv.should.equal(testAmount);
    ...
    object.outputs[0].glv.should.equal(testAmount - 10000);
  });

  it('will not accept NaN as an amount', () => {
    (function () {
      const stringTx = new Transaction().to('Lfci7ooSc31oNijNG9zDeHBBHJddxrPEKX', NaN);
  ...
  it('toObject/fromObject with signatures and custom fee', () => {
    const tx = new Transaction()
      ...
      .to([{address: toAddress, glv: 50000}])
      .fee(15000)
  ...
  var p2shPrivateKey1 = PrivateKey.fromWIF('2ShLGGW7NPNdTJsYKkVxx8Wp5ThyASmpumDKkj5P8iCGia7s3yXY3AnhCQRs9kQeYEoHjGHgmDCLhynE2RZPfQYh8ej1YGK');
  var p2shPublicKey1 = p2shPrivateKey1.toPublicKey();
  var p2shPrivateKey2 = PrivateKey.fromWIF('2SV7QR25RCZAT6GeyhmTAw6Rrm4JSywAUKow9CWnY277c8BeHv7B2UNJPm3sG6zm8cSn2qNozy2yco6nVFMuy1q7ojNdsUB');
  var p2shPublicKey2 = p2shPrivateKey2.toPublicKey();
  const p2shPrivateKey3 = PrivateKey.fromWIF('2SWYiDzgzw1dmDh1SfZXi51kBUD6A1Qzncjf4Fgi1zm4LqxaM9qtRnLULeZjwMc97sfxwAka9j8Kz2d8VSwgFAKuj8AUgmg');
  ...
  describe('change address', () => {
    it('can calculate simply the output amount', () => {
      ...
      transaction.outputs[1].glv.should.equal(40000);
    ...
    it('can recalculate the change amount', function () {
      ...
      transaction.getChangeOutput().glv.should.equal(50000);
      ...
      transaction.outputs[2].glv.should.equal(30000);
    ...
    it('fee can be set up manually', () => {
      const transaction = new Transaction()
        ...
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].glv.should.equal(10000);
    });
    it('fee per kb can be set up manually', () => {
      ...
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].glv.should.equal(34000);
    });
    it('if glv are invalid', () => {
      const transaction = new Transaction()
        .from(simpleUtxoWith100000Satoshis)
        .to(toAddress, 99999)
        .change(changeAddress)
  ...
  describe('checked serialize', () => {
    ...
    describe('skipping checks', () => {
      ...
      it('can skip the check for too much fee', buildSkipTest((transaction) => {
        return transaction
          .fee(50000000)
          .change(changeAddress)
          .sign(privateKey);
      }, 'disableLargeFees', errors.Transaction.FeeError.TooLarge));
      it('can skip the check for a fee that is too small', buildSkipTest((transaction) => {
        return transaction
          .fee(1)
          .change(changeAddress)
          .sign(privateKey);
      }, 'disableSmallFees', errors.Transaction.FeeError.TooSmall));
      it('can skip the check that prevents dust outputs', buildSkipTest((transaction) => {
        return transaction
          .to(toAddress, 100)
          .change(changeAddress)
          .sign(privateKey);
      }, 'disableDustOutputs', errors.Transaction.DustOutputs));
      it('can skip the check that prevents unsigned outputs', buildSkipTest((transaction) => {
        return transaction
          .to(toAddress, 10000)
          .change(changeAddress);
  ...
  describe('#verify', () => {
    it('not if _satoshis and _satoshisBN have different values', () => {
      ...
      verify.should.equal('transaction txout 0 glv is invalid');
    });
  ...
  describe('removeInput and removeOutput', () => {
    it('can remove an input by index', () => {
      ...
      transaction.inputAmount.should.equal(simpleUtxoWith1BTC.glv);
    ...
    it('an output can be removed by index', () => {
      const transaction = new Transaction()
        .to([
          {address: toAddress, glv: 40000000},
          {address: toAddress, glv: 40000000}
        ])
      transaction.outputs.length.should.equal(2);
  ...
  describe('output ordering', () => {
    ...
    beforeEach(() => {
      transaction = new Transaction()
        .from(simpleUtxoWith1BTC)
        .to([
          {address: toAddress, glv: tenth},
          {address: toAddress, glv: fourth}
        ])
        .to(toAddress, half)
        .change(changeAddress);
  ...
  describe('clearOutputs', () => {
    it('removes all outputs and maintains the transaction in order', () => {
      ...
      tx.outputs[0].glv.should.equal(10000000);
      tx.outputs[0].script.toAddress().toString().should.equal(toAddress);
      tx.outputs[1].glv.should.equal(89990000);
  ...
  describe('BIP69 Sorting', () => {
    ...
    it('sorts outputs correctly', () => {
      ...
      tx.sort();
      tx.outputs[0].glv.should.equal(1);
      tx.outputs[1].glv.should.equal(2);
      tx.outputs[2].glv.should.equal(2);
        ...
        it(inputSet.description, () => {
          ...
          inputSet.inputs = inputSet.inputs.map((input) => {
            var input = new Input({
              ...
              script: new Script(),
              output: new Output({ script: new Script(), glv: 0 })
            });
    ...
    describe('#isRBF', () => {
      ...
      it('determine opt-out with 0xfffffffe', () => {
        const tx = new Transaction()
          .from(simpleUtxoWith1BTC)
          .from(simpleUtxoWith100000Satoshis)
          .to([{address: toAddress, glv: 50000 + 1e8}])
          .fee(15000)
          .change(changeAddress)
          .sign(privateKey);
